...
class AdaptVQE():
    ...
    def evaluate_energy_linalg(
            self, coefficients=None,indices=None,ref_state=None
    ):
        if indices is None:
        ...
        observable = get_sparse_operator(self.qubit_hamiltonian, self.n)
    ...
    def estimate_gradient(
            self, operator_pos=None,coefficients=None,indices=None,method='fd',dx=10**-8,ref_state=None
    ):
        if indices is None:
            assert coefficients is None
            indices = self.indices.copy()
        ...
        coefficients_plus += dx

        energy = self.evaluate_energy_linalg(coefficients, indices, ref_state)
        energy_plus = self.evaluate_energy_linalg(coefficients_plus, indices, ref_state)
    ...
    def estimate_snd_derivative(
            self, op1_pos=None, op2_pos=None, coefficients=None,indices=None, method='fd', formula='central', dx=10**-4
    ):
        if indices is None:
            assert coefficients is None
            indices = self.indices.copy()
        ...
        e_pp = self.evaluate_energy_linalg(coefficients_pp, indices)
        e_pm = self.evaluate_energy_linalg(coefficients_pp, indices)
        e_mp = self.evaluate_energy_linalg(coefficients_pp, indices)
        e_mm = self.evaluate_energy_linalg(coefficients_pp, indices)
    ...
    def estimate_snd_derivative_1var(
        ...
        energy = self.evaluate_energy_linalg(coefficients, indices)
